...
public class HashTable<K, V> {
    ...
    private String nameOfHashtable;
    ...
    public HashTable(int hashTableSize, String name) {
        setTable(new Hash[hashTableSize]);
        this.hashTableSize = hashTableSize;
        size = 0;
        nameOfHashtable = name;
    }
    ...
    private void ensureCapacity() {
        if (size + 1 > hashTableSize / 2) {
            ...
            for (int i = 0; i < table.length; i++) {
                if (table[i] != null && !table[i].tomb) {
                    int index = this.findSlot(table[i].getKey(), table2);
                    table2[index] = table[i];
                }
            }
            table = table2;
            System.out.println(nameOfHashtable + " hash table size doubled to "
                + hashTableSize + " slots.");
        }
    }
    ...
    /**
     * according to the value given find its store position
     * 
     * @param value
     *            the value need to be found
     * @return the index of the location -1 if not found
     */
